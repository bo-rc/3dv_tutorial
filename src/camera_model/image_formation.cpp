...
int main()
{
    ...
    for (size_t i = 0; i < cam_pos.size(); i++)
    {
        ...
        x.row(2) = 1; // when this == 1, x.row(0) and x.row(1) corresponds to image plane at f.
                      // refer to: normalized_plane.png  
        // should be the same result as the following:
        // cv::Mat X_ = Rt * X;
        // X_.row(0) = X_.row(0) / X_.row(2);
        // X_.row(1) = X_.row(1) / X_.row(2);
        // X_.row(2) = 1;
        // cv::Mat x = K * X_;
